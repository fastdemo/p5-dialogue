...
const ImageCanvas = ({ activeGame, char, emote, costume, name, text, font, portrait, custom, boxType, setCustom }: any) => {
  ...
  const redraw = () => {
    ...
    if (isP5) {
      ...
      /* Step 4: Name box image (unrotated) */
      ...
      /* Step 4b: Tiles + name text (rotated via ASYNC font load) */
      ...
      /* Step 3: Box back (dialogue bubble background) — drawn FIRST behind portrait */
      ...
      /* Step 4: Portrait (on top of box back) */
      if (char !== 'None') {
      ...
      /* Step 5: Box front (name plate frame) — on top of portrait */
      const nb = nameBubbleRef.current;
      if (nb && nb.complete && nb.naturalWidth > 0) {
      ...
      /* Step 6: Name + dialogue text (on top of everything) */
      loadedFont.load().then(() => {
        ctx.font = `26pt ${font}`;
        if (boxType === 'golden') {
